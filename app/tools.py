"""
Database query tools untuk LangChain Agent
Hanya file ini yang berinteraksi dengan database
"""
from sqlalchemy import text
from langchain_core.tools import tool
from models import db, Kursi, Jadwal
...
def get_schedule_info(jadwal_id: int) -> dict:
    ...
    try:
        schedule = db.session.query(Jadwal).filter(
            Jadwal.Id_Jadwal == jadwal_id
        ).first()
        
        if not schedule:
            return {"error": "Jadwal tidak ditemukan"}
        
        available_count = db.session.query(Kursi).filter(
            Kursi.Id_Jadwal == jadwal_id,
            Kursi.Status_Pemesanan == 0
        ).count()
...
def get_all_schedules() -> list:
    ...
    try:
        ...
        schedules = db.session.query(Jadwal).filter(
